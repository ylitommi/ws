...
class CountUntilClient(Node):
    ...
    def goal_feedback_callback(self, feedback_msg):
        ...
        self.get_logger().info("Got feedback from action server: " + str(number))
        # Let's call cancel_goal method to demonstrate canceling of an action goal
        if number >= 4:
            self.cancel_goal()
    ...
    def cancel_goal(self):
        self.get_logger().info("Send a cancel goal request to action server.")
